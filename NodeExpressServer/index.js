...
//Returns list of all the foodData which contains vegetables
app.get("/vegetable", (req, res) => {
    const vegetables = foodData.filter(food => food.category === "Vegetable");
    res.json(vegetables);
  });

//Returns list of all the foodData which contains fruits
app.get("/fruits", (req, res) => {
    const fruits = foodData.filter(food => food.category === "Fruit");
    res.json(fruits);
  });

//Returns list of all the foodData which contains proteins
  app.get("/protein", (req, res) => {
    const proteins = foodData.filter(food => food.category === 'Protein');
    res.json(proteins);
  });

//Returns list of all the foodData which contains calories more than 100
app.get("/calorie-above-100", (req, res) => {
    const Calabove100 = foodData.filter(food => food.calorie > 100);
    res.json(Calabove100);
  });

//Returns list of all the foodData which contains calories less than 100
app.get("/calorie-below-100", (req, res) => {
    const Calbelow100 = foodData.filter(food => food.calorie < 100);
    res.json(Calbelow100);
  });



//Return health-api data
